...
const AddProduct = () => {
  ...
  const handleViewBooking = (userId, userName) => {
    navigate(
      `/admin/userevents/${userId}?name=${encodeURIComponent(userName)}`
    );
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      setUsers(
        users.map((user) => (user._id === currentUser._id ? currentUser : user))
      );
      handleCloseModal();
  ...
  const handleConfirmDelete = async () => {
    if (selectedUser) {
      ...
      await axios
        .delete(url + "/user/" + selectedUser._id)
        .then((resp) => {
          // console.log(resp);
    ...
    <div className="container gap-3">
                  ...
                  <tr key={user._id}>
                    ...
                    <td>
                      {user.events.length > 0 ? (
                        <Events
                          titleAccess="View User Events"
                          className="cursor-pointer"
                          onClick={() => handleViewBooking(user._id, user.name)}
                          style={{ color: "#D26600" }}
                        />
                      ) : (
                        <p>NA</p>
      ...
      >
        ...
        <Modal.Body>
          ...
          <p className="text-danger text-uppercase fw-bolder">
            Deleting: {selectedUser && selectedUser.name}
          </p>
        </Modal.Body>
        <Modal.Footer>
